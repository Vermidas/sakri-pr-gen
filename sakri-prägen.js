...
(function() {
  ...
  const praegeSettings = {
    ...
    zeitAnfordern: storage.get('prägeAnfordern', '0'),
    ...
    <table id="praegeTable" class="vis overview_table">
      ...
      <tbody>
        ...
        <div style="display: flex; justify-content: center; align-items: center; font-weight: bold;">
          Prägen
          <input type="checkbox" id="praegeCheckbox" style="margin-left: 10px;">
        </div>
      </th>
        ...
        <div style="display: flex; justify-content: center; align-items: center; font-weight: bold;">
          Anfordern
          <input type="checkbox" id="anfordernCheckbox" style="margin-left: 10px;">
        </div>
      </th>
        ...
        <tr>
          <td colspan="2" style="text-align: center;">
            ...
            <button class="btn" id="löschenButton">Zurücksetzen</button>
          </td>
        </tr>
      </tbody>
    </table>
...
function toggleScript(start) {
  // Überprüfung, ob die Checkbox ausgewählt ist
  const isCheckboxChecked = document.getElementById('praegeCheckbox').checked;
  if (!isCheckboxChecked) {
    console.log('Script wird nicht gestartet, da Checkbox nicht ausgewählt ist.');
    return; // Frühes Beenden der Funktion, wenn Checkbox nicht ausgewählt ist
  }
...
document.getElementById('anfordernzeit').addEventListener('input', (e) => {
  ...
  if (value < 60) {
    ...
    storage.set('prägeAnfordern', value);
    praegeSettings.zeitAnfordern = value;
  }
});

document.getElementById('praegeCheckbox').addEventListener('change', function() {
  localStorage.setItem('prägeCheckboxChecked', this.checked.toString());
});

const isCheckedPrägen = localStorage.getItem('prägeCheckboxChecked') === 'true';
document.getElementById('praegeCheckbox').checked = isCheckedPrägen;

document.getElementById('anfordernCheckbox').addEventListener('change', function() {
  localStorage.setItem('anfordernprägeCheckboxChecked', this.checked.toString());
});

const isCheckedAnfordern = localStorage.getItem('anfordernprägeCheckboxChecked') === 'true';
document.getElementById('anfordernCheckbox').checked = isCheckedAnfordern;
...
document.getElementById('löschenButton').addEventListener('click', function() {
  ...
  console.log('Alle Schlüssel mit "prägen" wurden aus dem localStorage entfernt.');
});
...
function addCoordToPraegeTable(coord, index) {
  ...
  const newRow = tbody.insertRow(4); // Position kann je nach Bedarf angepasst werden
  ...
  // Hinzufügen einer Standardoption als Aufforderung für den Benutzer
  ...
  groupInfoList.forEach(group => {
    ...
    const savedGroupId = localStorage.getItem(`selectedGroupForPrägen_${coord}`);
  ...
  selectElement.addEventListener('change', async function() {
    ...
    localStorage.setItem(`selectedGroupForPrägen_${coord}`, selectedGroupId);
...
function startCountdown() {
  const isCheckboxChecked = document.getElementById('anfordernCheckbox').checked;

  if (!praegeSettings.running || !isCheckboxChecked) {
...
document.addEventListener('DOMContentLoaded', (event) => {
  if (localStorage.getItem('prägeAnfordern')) {
    createCountdownDisplay();
    startCountdown();
  }
});
